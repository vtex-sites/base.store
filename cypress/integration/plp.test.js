...
describe('Infinite Scroll pagination', () => {
  ...
  it('Sticks to last seen page on plp pagination', () => {
    ...
    cy.getById('product-link')
      ...
      .should('have.length.gt', 0)
      .then(($links) => {
        const before = $links.length // number of products before showMore is clicked

        cy.getById('show-more')
          .should('exist')
          .click()
          .then(() => {
            let skuIdBeforeNavigate // The skuId of the last product on the page

            cy.getById('product-link')
              .should('have.length.gte', before) // number of products after showMore is clicked should be higher
              .last()
              .within(() => {
